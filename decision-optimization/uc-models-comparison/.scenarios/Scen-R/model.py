...
class UnitCommitment(object):
    ...
    def __prepare_plan_df(df_used, df_started, df_stopped):
        df_plan = df_used.rename(columns={'value': 'active'}).merge(df_started.rename(columns={'value': 'on'}),
                                                                    on=['period', 'unit'])
    ...
    def __prepare_reserve_kpi_df(kpis_bd):
        def extract_period_reserve_kpi(kpi_name):
            kpi_df = kpis_bd.loc[kpis_bd['kpi'].str.contains(kpi_name)].copy()
    ...
    def solve(self):
        if self.uc.solve():
            ...
            kpis_bd = kpis_bd.append(
                {'kpi': 'cplex time', 'value': "{:.3f}".format(self.uc.get_solve_details().time)}, ignore_index=True)
...
class StochasticUnitCommitment(UnitCommitment):
    ...
    def import_data(self, sc_inputs):
        ...
        self.params['Num_SCEN_P_ELNS'] = int(
            len(self.params['scenarios']) * (1 - self.df_scenarios_conf.PROB_Req_ELNS[0]) + 1)
        self.params['Num_SCEN_P_EWS'] = int(
            len(self.params['scenarios']) * (1 - self.df_scenarios_conf.PROB_Req_EWS[0]) + 1)

    def build_stochastic_vars(self, periods):
        scenarios = self.params['scenarios']
        stoch_res_up = self.uc.continuous_var_matrix(keys1=scenarios, keys2=periods, lb=-self.uc.infinity, \
                                                     name="stoch_res_up")
        stoch_res_down = self.uc.continuous_var_matrix(keys1=scenarios, keys2=periods, lb=-self.uc.infinity, \
                                                       name="stoch_res_down")
    ...
    def build_var_expressions(self):
        ...
        for period in periods:
            elns_expr_list = [self.uc.max(df_join_scenario_vars_up.stoch_res_up[s, period], 0) for s in
                              self.params['scenarios']]
            self.stoch_res_up_tot[period] = self.params['PROB_SCEN'] * self.uc.sum(elns_expr_list)
            ews_expr_list = [self.uc.max(df_join_scenario_vars_up.stoch_res_down[s, period], 0) for s in
                             self.params['scenarios']]
            self.stoch_res_down_tot[period] = self.params['PROB_SCEN'] * self.uc.sum(ews_expr_list)

            if 'CVaR' in self.criteria:
                n_scen = len(self.params['scenarios'])
                n_scen_p_elns = self.params['Num_SCEN_P_ELNS']
                n_scen_p_ews = self.params['Num_SCEN_P_EWS']
                elns_expr_list_p = [self.uc.max(df_join_scenario_vars_up.stoch_res_up[s, period], 0) for s in
                                    range(n_scen - n_scen_p_elns, n_scen)]
                self.stoch_res_p_up_tot[period] = self.params['PROB_SCEN'] * self.uc.sum(elns_expr_list_p)
                ews_expr_list_p = [self.uc.max(df_join_scenario_vars_up.stoch_res_down[s, period], 0) for s in
                                   range(n_scen - n_scen_p_ews, n_scen)]
                self.stoch_res_p_down_tot[period] = self.params['PROB_SCEN'] * self.uc.sum(ews_expr_list_p)
    ...
    def add_stoch_model_cons(self):
        ...
        periods = self.df_load.index.values.tolist()
        for period in periods:
            if 'Max_ELNS' in self.criteria:
                self.uc.add_constraint(self.stoch_res_up_tot[period] <= self.df_scenarios_conf.ELNS_Max[0])
                self.uc.add_constraint(self.stoch_res_down_tot[period] <= self.df_scenarios_conf.EWS_Max[0])
            if 'CVaR' in self.criteria:
                self.uc.add_constraint(self.stoch_res_p_up_tot[period] <= self.df_scenarios_conf.ELNS_Max_P[0])
                self.uc.add_constraint(self.stoch_res_p_down_tot[period] <= self.df_scenarios_conf.EWS_Max_P[0])

    def add_objective_and_kpis(self):
        super(StochasticUnitCommitment, self).add_objective_and_kpis()
        elns_cost = self.uc.sum([self.df_scenarios_conf.Price_ELNS[0] * i for i in self.stoch_res_up_tot.values()])
        ews_cost = self.uc.sum([self.df_scenarios_conf.Price_EWS[0] * i for i in self.stoch_res_down_tot.values()])
        self.obj += elns_cost + ews_cost
        self.uc.minimize(self.obj)
        self.uc.add_kpi(elns_cost, "Stoch Vars Res Up Value")
        self.uc.add_kpi(ews_cost, "Stoch Vars Res Down Value")
